...
export class FileContentProcessor {
  ...
  private async storeFileEmbeddings(fileRecord: any, extractedContent: string): Promise<void> {
    try {
      // Import VectorService dynamically to avoid circular dependencies
      const { default: vectorServiceModule } = await import('../app/api/chat/stream/route');
      
      // Get user ID from the file record
      const { data: messageRecord } = await this.supabase
        .from('messages')
        .select('*')
        .eq('id', fileRecord.message_id)
        .single();

      if (!messageRecord) {
        throw new Error('Could not find associated message for file');
      }

      // Create a simple VectorService instance
      const vectorService = {
        ...
        chunkContent(content: string, maxChunkSize: number = 8000): string[] {
      ...
      const chunks = vectorService.chunkContent(extractedContent);
      
      console.log(`📊 Generating embeddings for ${chunks.length} chunks from file: ${fileRecord.original_name}`);
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const { error } = await this.supabase
          ...
          .insert({
            ...
            metadata: {
              original_filename: fileRecord.original_name,
      ...
      console.log(`🎉 Successfully stored ${chunks.length} embeddings for file: ${fileRecord.original_name}`);

    } catch (error) {
      console.error('Error storing file embeddings:', error);
      throw error;
    }
  }
